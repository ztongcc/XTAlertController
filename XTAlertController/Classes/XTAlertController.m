...
@interface XTAlertController ()

@property (nonatomic, strong) UIView *alertView;

@property (nonatomic, assign) XTAlertControllerStyle preferredStyle;

@property (nonatomic, assign) XTAlertTransitionAnimation transitionAnimation;
...
@implementation XTAlertController
...
- (instancetype)initWithAlertView:(UIView *)alertView preferredStyle:(XTAlertControllerStyle)preferredStyle transitionAnimation:(XTAlertTransitionAnimation)transitionAnimation transitionAnimationClass:(Class)transitionAnimationClass navigationClass:(Class)navigationClass {
    if (self = [self initWithNibName:nil bundle:nil]) {
        _alertView = alertView;
        _preferredStyle = preferredStyle;
        _transitionAnimation = transitionAnimation;
        _transitionAnimationClass = transitionAnimationClass;
        _navigationControllerClass = navigationClass;
...
+ (instancetype)alertControllerWithAlertView:(UIView *)alertView {
    return [[self alloc] initWithAlertView:alertView preferredStyle:XTAlertControllerStyleAlert transitionAnimation:XTAlertTransitionAnimationFade transitionAnimationClass:nil navigationClass:nil];
}

+ (instancetype)alertControllerWithAlertView:(UIView *)alertView preferredStyle:(XTAlertControllerStyle)preferredStyle {
    return [[self alloc] initWithAlertView:alertView  preferredStyle:preferredStyle transitionAnimation:XTAlertTransitionAnimationFade
                 transitionAnimationClass:nil navigationClass:nil];
}
...
+ (instancetype)alertControllerWithAlertView:(UIView *)alertView preferredStyle:(XTAlertControllerStyle)preferredStyle transitionAnimationClass:(Class)transitionAnimationClass {
    return [[self alloc] initWithAlertView:alertView preferredStyle:preferredStyle transitionAnimation:XTAlertTransitionAnimationCustom
                  transitionAnimationClass:transitionAnimationClass navigationClass:nil];
}
...
+ (instancetype)alertControllerWithAlertView:(UIView *)alertView preferredStyle:(XTAlertControllerStyle)preferredStyle
    ...
    return [[self alloc] initWithAlertView:alertView preferredStyle:preferredStyle transitionAnimation:XTAlertTransitionAnimationFade
                  transitionAnimationClass:nil navigationClass:navigationClass];
}
...
+ (instancetype)alertControllerWithAlertView:(UIView *)alertView preferredStyle:(XTAlertControllerStyle)preferredStyle transitionAnimationClass:(Class)transitionAnimationClass navigationClass:(Class)navigationClass {
    return [[self alloc] initWithAlertView:alertView preferredStyle:preferredStyle transitionAnimation:XTAlertTransitionAnimationCustom
                  transitionAnimationClass:transitionAnimationClass navigationClass:navigationClass];
}
...
- (void)viewDidLoad {
    ...
    if (_preferredStyle == XTAlertControllerStyleAlert) {
...
- (void)updateConfigValues {
    ...
    if (_navigationControllerClass) {
        _embedInNavigationController = YES;
    }
}
...
- (void)layoutDrawerStyleView {
    ...
    CGFloat off = CGRectGetWidth(self.view.bounds) - CGRectGetWidth(self.alertView.frame);
    // add edge constraint
    if (_preferredStyle == XTAlertControllerStyleDrawerFromLeft) {
        [self.view addConstraintWithView:_alertView topView:nil leftView:self.view bottomView:self.view rightView:self.view edgeInset:UIEdgeInsetsMake(0, 0, 0, -off)];
    }else {
        [self.view addConstraintWithView:_alertView topView:nil leftView:self.view bottomView:self.view rightView:self.view edgeInset:UIEdgeInsetsMake(0, off, 0, 0)];
    }
...
- (UINavigationController *)buildNavigationController:(UIViewController *)rootViewController {
    UINavigationController * navigationController;
    if (_navigationControllerClass) {
        navigationController = [[_navigationControllerClass alloc] initWithRootViewController:rootViewController];
    }else {
        navigationController = [[UINavigationController alloc] initWithRootViewController:rootViewController];
    }
    if (_navigationControllerSetupHandler) {
        _navigationControllerSetupHandler(navigationController);
    }
...
- (void)showFromController:(UIViewController *)fromViewController animated:(BOOL)flag completion:(void (^)(void))completion {
    UIViewController * showVC = self;
    if (_embedInNavigationController) {
        showVC = [self buildNavigationController:self];
    }
    UIViewController * fromVC = fromViewController;
...
//        UIView * contentView = self.alertView;
//        TYShowAlertView * alertView = [TYShowAlertView alertViewWithView:contentView];
//        alertView.backgoundTapDismissEnable = YES;
//        fromVC = [((UINavigationController *)fromVC).viewControllers lastObject];
//        [alertView showInController:fromVC];
//    }else {
        [fromVC presentViewController:showVC animated:flag completion:completion];
//    }
}
